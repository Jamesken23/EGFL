...
from tensorflow.keras.layers import (Dense, Embedding, SpatialDropout1D, ReLU, GlobalAveragePooling1D, Conv1D,
                                     MaxPooling1D, Dropout, Layer, LayerNormalization, BatchNormalization)
...
"""
Similar to TextCNN + Transformer network
...
class Attention(tf.keras.layers.Layer):
    def __init__(
        self, dim, heads=10, dim_head=64, dropout=0.0, max_pos_emb=512, **kwargs
    ):
        super(Attention, self).__init__(**kwargs)
        inner_dim = dim_head * heads * 4/5
        self.heads = heads
...
class egfl_model:
    def __init__(self, cfg_bfs_data, graph_data, args, log_set):
        ...
        x_conv1 = Conv1D(filters=args.CNN_LAYER_DIM, kernel_size=5, activation='relu', padding="same")(x_embedding)
